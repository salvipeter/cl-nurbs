...
(defun bsc-insert-knot (curve u)
  "Inserts the not U into the knot vector of CURVE."
  (with-accessors ((degree degree)
		   (knots knot-vector)
		   (points control-points))
      curve
    (let ((new-knots (sort (concatenate 'vector knots (list u)) #'<))
	  (new-points (make-array (1+ (length points)))))
      (let ((k (1- (position u new-knots :from-end t))))
	(iter (for i from 0 to (- k degree))
	      (setf (elt new-points i) (copy-list (elt points i))))
	(iter (for i from (+ k (- degree) 1) to k)
	      (setf (elt new-points i)
		    (affine-combine (elt points (1- i))
				    (/ (- u (elt knots i))
				       (- (elt knots (+ i degree))
					  (elt knots i)))
				    (elt points i))))
	(iter (for i from (1+ k) below (length new-points))
	      (setf (elt new-points i) (copy-list (elt points (1- i))))))
