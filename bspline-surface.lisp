...
(defun bsc-extrude (curve length &optional (by-degree 1))
...
CURVE by LENGTH units along the z axis."
  (with-accessors ((degree degree)
      ...
      curve
    (let ((net (map 'vector (lambda (x)
			      (append (list (append x (list 0)))
				      (iter (for i from 1 to by-degree)
					    (for z = (* (/ i (1+ by-degree))
							length))
					    (for p = (append x (list z)))
					    (collect p))
				      (list (append x (list length)))))
		    points))
	  (ex-knots (append (iter (repeat (1+ by-degree))
				  (collect 0))
			    (list 1/2)
			    (iter (repeat (1+ by-degree))
				  (collect 1)))))
      (make-bspline-surface (list degree by-degree)
			    (list (copy-seq knots) ex-knots)
			    (make-array (list (length points) (+ 2 by-degree))
...
(defun bss-principal-curvatures (surface uv)
  ...
  (let* ((der-00 (bss-evaluate surface uv :derivative '(1 0)))
	 ...
	 (a (* 2 (- (* E G) (* F F))))
	 (d (sqrt (- (* x x) (* a y)))))
    (cond ((complexp d) (list (/ x a) (/ x a)))	; umbilical point
	  ((< a 0) (list (/ (- x d) a) (/ (+ x d) a)))
	  (t (list (/ (+ x d) a) (/ (- x d) a))))))
