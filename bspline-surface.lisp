...
(defun bss-insert-knot-u (surface u)
  ...
  (with-accessors ((degrees degrees)
	...
	(iter (for i from 0 below u-degree)
	      (for j = (+ k (- u-degree) 1 i))
	      (setf (elt alpha i)
		    (/ (- u (elt u-knots j))
		       (- (elt u-knots (+ j u-degree)) (elt u-knots j)))))
	(dotimes (j (array-dimension net 1))
	  (iter (for i from 0 to (- k u-degree))
		(setf (aref new-net i j) (copy-list (aref net i j))))
	  (iter (for i from (+ k (- u-degree) 1) to k)
		(setf (aref new-net i j)
		      (affine-combine (aref net (1- i) j)
				      (elt alpha (- i k (- u-degree) 1))
				      (aref net i j))))
	  (iter (for i from (1+ k) below (array-dimension new-net 0))
		(setf (aref new-net i j)
...
(defun bss-insert-knot-v (surface v)
  ...
  (with-accessors ((degrees degrees)
	...
	(iter (for i from 0 below v-degree)
	      (for j = (+ k (- v-degree) 1 i))
	      (setf (elt alpha i)
		    (/ (- v (elt v-knots j))
		       (- (elt v-knots (+ j v-degree)) (elt v-knots j)))))
	(iter (for i from 0 below (array-dimension net 0))
	      (iter (for j from 0 to (- k v-degree))
		    (setf (aref new-net i j) (copy-list (aref net i j))))
	      (iter (for j from (+ k (- v-degree) 1) to k)
		    (setf (aref new-net i j)
			  (affine-combine (aref net i (1- j))
					  (elt alpha (- j k (- v-degree) 1))
					  (aref net i j))))
	      (iter (for j from (1+ k) below (array-dimension new-net 1))
		    (setf (aref new-net i j)
...
(defun bss-insert-knot (surface knot &key (u-direction t))
  "TODO: This should be made into one, simpler function."
  (if u-direction
